...
class Bearer implements TokenTypeInterface
{
    ...
    public function getAccessTokenParameter(RequestInterface $request, ResponseInterface $response)
    {
        ...
        $isCanGet = (isset($request->isCanGet)) ? $request->isCanGet : false;
        $headers = $request->headers('AUTHORIZATION');
        ...
        if ($request->request($this->config['token_param_name'])) {
            ...
            if (!in_array(strtolower($request->server('REQUEST_METHOD')), array('post', 'put')) && !$isCanGet) {
            ...
            if ($contentType !== null && $contentType != 'application/x-www-form-urlencoded' && !$isCanGet) {
