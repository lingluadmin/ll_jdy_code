            ...
            <dl class="t-center-left-6">
                ...
                @if( !empty($creditDetail['companyView']['credit_list_info']) )
                    ...
                    <table width="610">
                        <tr>
                            <td width="305">借款人姓名：
                                {{ \App\Tools\ToolStr::hidePhone( $credit_item['loan_username'], 3 ,3) }}
                            </td>
                            <td>借款人身份证号：
                                {{ \App\Tools\ToolStr::hidePhone( $credit_item['loan_user_identity'], 8, 4)}}
                                </td>
                        </tr>
                        <tr>
                            <td>借款金额：
                                {{ $credit_item['loan_amounts'] }} 元
                                </td>
                        </tr>
                    </table>
                   @endforeach
                        ...
                        <table class="table table-theadbg table-textcenter">
                            <tbody>
                                @if( !empty($creditDetail['companyView']['credit_list_info']) )
                                    @foreach($creditDetail['companyView']['credit_list_info'] as $credit_item)
                                        <tr>
                                            <td width="120"> {{ $credit_item['loan_username'] }} </td>
                                            <td width="170"> {{ $credit_item['loan_user_identity'] }}  </td>
                                            <td width="170"> {{ number_format($credit_item['loan_amounts'],2) }} </td>
                                            <td>{{ $credit_item['credit_name'] }} </td>
                                        </tr>
                                    @endforeach
