<div class="v4-wrap-margin clearfix" ms-if="@counter.len1 > 0">
    <div class="v4-project-more smart">
        <h3>智投计划</h3>
        <p>智能匹配 资金分散</p>
        <a href="/project/index" class="v4-btn v4-btn-more">更多项目</a>
    </div>

    <div class="v4-project-box" ms-for="(k,v) in @project.smartProjectList" ms-click="@redirectDetail(@v.id)" >
        ...
        <label>锁定期限&nbsp;{%@v.format_invest_time%}&nbsp;{% @v.invest_time_unit%}</label>
        <a ms-if="@v.status==130" ms-attr="{'href':'/smartInvest/detail/'+@v.id}" class="v4-btn">立即出借</a>
        <a ms-if="@v.status==150" ms-attr="{'href':'/smartInvest/detail/'+@v.id}" class="v4-btn v4-btn-disabled">已售罄</a>
        <a ms-if="@v.status==160" ms-attr="{'href':'/smartInvest/detail/'+@v.id}" class="v4-btn v4-btn-disabled">已完结</a>
    </div>
</div>
