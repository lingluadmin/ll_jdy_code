...
class QueryListener
{
    ...
    public function handle(QueryExecuted $query)
    {
        ...
        $sql = str_replace("?", "'%s'", str_replace("%", "'%%'", '[ query ]: '.$query->sql) );
        ...
        $message = self::getSession().' ['.date('Y-m-d H:i:s').'] ' . $query . PHP_EOL;
        ...
        */
        ...
        //error_log(self::getSession().' ['.date('Y-m-d H:i:s').'] ' . $query . PHP_EOL,3,storage_path('logs' . DIRECTORY_SEPARATOR . 'query-'.date('Y-m-d') . '.log'));
