...
class ToolTime{
     ...
     * @return float
     */
    public static function getYearDiff($startTime, $endTime)
    {
        ...
        $diffYear = $startTime->diffInYears($endTime);
     ...
     * @param $dateTime
     * @return bool|string
     * @desc 获取某个时间的日期,格式化为点的日期
     */
    public static function getDotDate($dateTime)
    {
        return date("Y.m.d",strtotime($dateTime));
    }
     ...
     * @return array
     */
    public static function getDateList($format = 'Y-m-d', $start='', $end=''){
        $dateList = [];
        ...
        foreach($period as $key => $dateTime) {
            $dateList[$key]['format_date'] = $dateTime->format($format);
            $dateList[$key]['date'] = $dateTime->format('Y-m-d');
        }
        return $dateList;
    }
}
