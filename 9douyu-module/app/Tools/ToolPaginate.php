...
class ToolPaginate
{
    ...
    /**
     * @desc 分页问题信息逻辑处理[用于分页显示]
     * @param $nums 要显示的分页数据
     * @return array
     */
    public function getPagerInfo($nums = 10)
    {
        //获取数据初始化的分页信息
        $pager = $this->getPaginate();
        if($pager['last_page'] > $nums){
            if($pager['current_page'] <= $nums/2){
                $startPage = 1;
            }else{
                $startPage = $pager['current_page']-$nums/2;
            }
            $prefix = range($startPage, $pager['current_page']);
            $suffixLength = $nums-count($prefix);

            if(($pager['current_page']+$suffixLength) <= $pager['last_page']){
                $endPage  = $pager['current_page']+$suffixLength;
            }else{
                $endPage  = $pager['last_page'];
            }
            $pager['view'] = range($startPage,$endPage);
        }else{
            if($pager['last_page']>1){
                $pager['view'] = range(1,$pager['last_page']);
            }else{
                $pager['view'] = [];
            }
        }
