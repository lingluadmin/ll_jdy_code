...
/**
 * 债权九省心模型
 * Class CreditNineCreditModel
 * @package App\Http\Models\Credit
 */
class CreditNineCreditModel extends CreditModel implements CreditInterfaceModel
{
    ...
    public static $expNameSpace       = ExceptionCodeModel::EXP_MODEL_CREDIT_NINE;
    ...
    public static function doCreate($data){
        ...
        $return = CreditNineDb::addRecord($data);

        if(!$return)
            throw new \Exception(LangModel::getLang('ERROR_CREDIT_CREATE_NINE'), self::getFinalCode('doCreate'));
    ...
    public function getAdminList($condition = []){
        ...
        return CreditNineDb::select($field)->where($condition)->orderBy('id', 'desc')->paginate($size);
    }
    ...
    public static function doUpdate($id =0, $data = []){
        $return = CreditNineDb::doUpdate($id, $data);
        if(!$return)
