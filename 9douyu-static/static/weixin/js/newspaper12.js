...
$(document).swipeUp(function(){
	...
	last.row = now.row;
	last.col = now.col;
	if (last.row != 23) { now.row = last.row+1; now.col = 1; pageMove(towards.up);}	
	if(last.row == 23){ now.row = 1; now.col = 1; pageMove(towards.up);}
...
$(document).swipeDown(function(){
	...
	if(last.row == 1){ now.row = 23; now.col = 1; pageMove(towards.down);}	
})
...
function $$(name) {
    return document.getElementById(name);
        ...
        function playbksound() {
            ...
            bg_htm = "<img id='sound_image' src='/static/weixin/images/w12-music.png' class='img-icon-1 play'>";
