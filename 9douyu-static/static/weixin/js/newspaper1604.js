...
$(document).swipeUp(function(){
	...
	last.row = now.row;
	last.col = now.col;
	if (last.row != 17) { now.row = last.row+1; now.col = 1; pageMove(towards.up);}
	if(last.row == 17){ now.row = 1; now.col = 1; pageMove(towards.up);}
...
$(document).swipeDown(function(){
	...
	if(last.row == 1){ now.row = 17; now.col = 1; pageMove(towards.down);}
})
...
function $$(name) {
    return document.getElementById(name);
        ...
        function playbksound() {
            ...
            bg_htm = "<img id='sound_image' src='/static/weixin/images/news1604/news04-play.png' class='img-icon-1 play'>";
            sound_div.innerHTML = bg_htm ;
            document.body.appendChild(sound_div);
        }
		//播放音频文件
		gSound = '/static/weixin/newspaper1604.mp3';
