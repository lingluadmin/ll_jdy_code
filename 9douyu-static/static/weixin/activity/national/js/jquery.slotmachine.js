...
(function($) {
	...
	$(document).ready(function(){
		...
		if( $("filter#slotMachineBlurSVG").length<=0 ){
			$("body").append('<div style="position: absolute;top:0;z-index: -1;"><svg version="1.1" xmlns="http://www.w3.org/2000/svg">'+
	...
	$.fn.slotMachine = function(settings){
		...
		*/
		function _resetPosition(){
			$container.css("margin-top", _getOffset(_active.stopIndex));
		}
		...
		function _shuffle( count ){
			...
			//Stop animation after {count} repeats
				...
				if( count>=1 ){
					...
					//Stop NOW!
					//_stop(true);
					//if(typeof settings.stopIndex === count)
...
//ƒ						_stop(true);
		...
		*/
		...
		function  _appolit(){
			...
			var appIndex = settings.stopIndex;
