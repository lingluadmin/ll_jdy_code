...
function roll(){
    ...
    //执行抽奖的js 方法
var lotteryEvent    =   {
    error_img: '/static/weixin/activity/lottery/images/lottery/pop-error.png',
    baseObj:    'iphone8-lottery-main',
    thank_img:'/static/weixin/activity/lottery/images/lottery/pop-thanks.png',
        ...
        $.ajax({
            ...
            success : function(json) {
                ...
                if( json.status == true ) {
                    ...
                    if(lotteryType ==5) {
                        $('.' + _this.baseObj).attr('user_traffic_status','thank');
                    }else {
    ...
    successLet:function (traffic,type,index) {
        var lotteryObj      =   $('#btn-lottery-vip');
        lotteryObj.attr('lottery-active-prize',index);
        var _this   =   this;
        if( type == 5 ) {
            dialogHtml  =   _this.dialogLayer('thank', '');
            _this.errorLet(errorType, dialogHtml);
    ...
    dialogLayer:function ( type,information ) {
        ...
        switch (type){
            case 'notLogged':
                ...
                break;
            case 'notInTime':
                dialogHtml  =   _this.toInTimeDialog(_this,information);
                break;
            case 'thank':
                dialogHtml  =   _this.toThankDialog(_this);
                break;
            default:
    ...
    toInTimeDialog:function (_this, information) {
    ...
    toThankDialog:function (_this) {
        dialogHtml     ='<p><img src="'+_this.staticBaseUrl() + _this.thank_img+ '" class="pop-thanks-img"></p>'+
                        '<p>谢谢参与</p>' +
                        '<a href="javascript:;" data-toggle="mask" data-target="page-layer-error" class="page-pop-btn2">知道了</a>';
        return  dialogHtml;
    },
    ...
    toSuccessDialog:function (name) {
        dialogHtml  =   '<h4>恭喜啦</h4>'+
                        '<p>抽中'+name+'</p>'+
                        '<a href="/user" data-toggle="mask" data-target="page-layer-lottery" class="page-pop-btn btn2">使用九斗鱼账号查看</a>';
        return dialogHtml;
    },
    doLock:function (id) {
