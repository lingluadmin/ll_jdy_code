...
$(document).ready(function(){
    ...
    $("input[type=text],input[type=password]").each(function(){
        $(this).focus(function(){
            $("#tipMsg").addClass("focus").text($(this).attr("role-value")).show();
        }).blur(function(){
            $("#tipMsg").removeClass("focus").hide();
        });
    });
   ...
   //        $("#tipMsg").addClass("error").show();
   //    }else{
   //        $(this).attr("error", false);
   //        $("#tipMsg").removeClass("error").hide();
   //    }
   //});
    ...
    $("input[name=identity_card]").blur(function() {
        ...
        if(!checkIdCard(card)) {
            $('#yourCode').hide();
            $(this).attr("error", true);
            $("#tipMsg").text("身份证校验失败").addClass("error").show();
            return false;
        } else {
            $(this).attr("error", false);
            $("#tipMsg").removeClass("error").hide();
    ...
    function checkForm(){
        ...
        if($.trim($("input[name=real_name]").val()) == '') {
            $("#tipMsg").text("请填写您的名字").addClass("error").show();
            return false;
        }
        ...
        $.ajax({
            ...
            success:function(data){
                if(data.err==2){
                    $('#yourCode').hide();
                    $("#tipMsg").text("身份证已被认证").addClass("error").show();
                    sign = false;
                }else{
                    sign = true;
                }
    ...
    $("form").submit(function() {
        ...
        $("input").each(function(){
            if($(this).val() == '') {
                $("#tipMsg").addClass("error").text(textArr[$(this).attr("name")]).show();
                failFlag = true;
                return false;
            }else if($(this).attr("error") == "true"){
                $("#tipMsg").addClass("error").text($(this).attr("role-value")).show();
                failFlag = true;
                return false;
            }
        });
