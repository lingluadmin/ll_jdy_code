...
manager_config_file="$devops_prj_path/developer-name.config"
...
function _try_load_config() {
    ...
    if [ -z  "$developer_name" ]; then
        echo 'developer_name is not found, please call `sh manager.sh init_dev developer_name` first. ';
        exit 1
    fi
...
function init_dev() {
    if [ -z  "$developer_name" ]; then
        echo "Please input developer_name";
        exit 1
    fi
