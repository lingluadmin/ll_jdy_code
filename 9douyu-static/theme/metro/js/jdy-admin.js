...
function widthFunctions(e) {
	 ...
	 * 还款方式与期限逻辑
	 */
	$("body").on("change", "#selectRepayment", function(){
			var $code = $(this).val();
			if($code == 10){
				$('#loan_deadline').text('天');
			}else{
				$('#loan_deadline').text('月');
			}
	});
	...
	$("body").on("change", "#type", function(){
		...
		if($code == 200){
			$("#project_type_jdy").hide();
			$("#current_day").show();
		}else{
			$("#project_type_jdy").show();
			$("#current_day").hide();
		}
	});
	 ...
	 * 红包生效类型
	 */
	$("body").on("change", "#effect_type", function(){
		var $code = $(this).val();
		if($code == 200){
			$(".effect_time").show();
			$(".effect_day").hide();
